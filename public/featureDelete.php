...
function ciniki_marketing_featureDelete(&$ciniki) {
    ...
    $rc = ciniki_marketing_checkAccess($ciniki, $args['tnid'], 'ciniki.marketing.featureDelete'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }   
    ...
    if( !isset($rc['item']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.marketing.16', 'msg'=>'Feature does not exist'));
    }
    $item = $rc['item'];
    ...
    // Delete the object
    //
    return ciniki_core_objectDelete($ciniki, $args['tnid'], 'ciniki.marketing.feature', $args['feature_id'], $item['uuid'], 0x07);
}
?>
