...
function ciniki_marketing_featureImageAdd(&$ciniki) {
    ...
    $rc = ciniki_marketing_checkAccess($ciniki, $args['business_id'], 'ciniki.marketing.imageAdd'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    } 
	...
	if( $rc['stat'] != 'ok' ) {
		return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'1738', 'msg'=>'Unable to get a new UUID', 'err'=>$rc['err']));
	}
	$args['uuid'] = $rc['uuid'];
	...
	if( $rc['num_rows'] > 0 ) {
		return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'1739', 'msg'=>'You already have an image with this name, please choose another name'));
	}


	if( $args['feature_id'] <= 0 ) {
		return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'1740', 'msg'=>'No feature specified'));
	}
